...
users.forEach(({ first_name, last_name, age, email, avatar, progress }) => {
  ...
  cardsElem.classList.add('user_card');

  // cardsElem.style.backgroundColor = age >= 18 && first_name[0].toLowerCase() === 'a' ?  'lightpink' : 'lightblue';
  cardsElem.style.backgroundColor = age >= 18 && first_name[0].toLowerCase() === 'a' ? 'lightpink' : 'lightblue';

  progressLine.style.width = progress + '%';

  progressContainer.append(progressLine, progressValue);
  cardsElem.append(avatarElem, nameElem, ageElem, emailTextElem, emailElem, progressContainer);
  container.append(cardsElem);
})
